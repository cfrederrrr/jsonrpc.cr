...
class JSON::RPC::Response(R)
  ...
  #
  # `result` is nilable even if `R` is not; JSON::RPC 2.0 specification
  ...
  # Serverside can create a new `Response(R)` with direct arguments, rather than with a pullparser
  def initialize(@error : JSON::RPC::Error, @id : SID = nil, @jsonrpc = "2.0")
    @result = nil
  end
end
